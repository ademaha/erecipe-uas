...
export default class Boiler extends Component {
  ...
  this.itemsRef = firebase.database().ref('dessert');
  ...
  renderRow(item){
    return(
      <View style={{flexDirection:'column'}}>
        <Image source={require('./desert.jpg')} style={{width : 100 , height : 100}}>
        </Image>
  ...
  pressRow(item) {        
    ...
    firebase.database().ref('dessert/'+item).on('value', (snap) => {
      this.setState({
  ...
  deleteRow(item) {
    firebase.database().ref('dessert').child(item).remove();
                    ...
                    <TouchableHighlight
                      ...
                      onPress = {() => {
                        if(this.state.Nama == "" || this.state.Bahan == "" || this.state.Resep == "") {
                          ...
                          firebase.database().ref('dessert').child(this.state.setItem).update({
