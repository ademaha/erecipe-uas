...
export default class Boiler extends Component {
  ...
  this.itemsRef = firebase.database().ref('maincourse');
  ...
  renderRow(item){
    return(
      <View style={{flexDirection:'column'}}>
        <Image source={require('./maincourse.jpg')} style={{width : 100 , height : 100}}>
        </Image>
  ...
  pressRow(item) {        
    ...
    firebase.database().ref('maincourse/'+item).on('value', (snap) => {
      this.setState({
  ...
  deleteRow(item) {
    firebase.database().ref('maincourse').child(item).remove();
                    ...
                    <TouchableHighlight
                      ...
                      onPress = {() => {
                        if(this.state.Nama == "" || this.state.Bahan == "" || this.state.Resep == "") {
                          ...
                          firebase.database().ref('maincourse').child(this.state.setItem).update({
...
AppRegistry.registerComponent("Mainadmin", () => Mainadmin);